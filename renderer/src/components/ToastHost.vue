...
<template>
  <aside class="vue-toast-stack" aria-label="Notifications" aria-live="polite">
    <TransitionGroup name="vue-toast">
      <article v-for="toast in store.items" :key="toast.id" class="vue-toast" :data-tone="toast.tone">
        ...
        <button class="vue-toast-close" type="button" aria-label="Dismiss notification" @click="store.remove(toast.id)"><span class="material-symbols-outlined">close</span></button>
      </article>
    </TransitionGroup>
  </aside>
</template>
