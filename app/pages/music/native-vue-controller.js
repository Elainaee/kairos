...
const neteaseDownloadTasks = new Map();
...
const likedMenuLabel = liked => liked ? tr('music.unliked', {}, 'Unliked') : tr('music.liked', {}, 'Liked');
...
const renderNeteaseSidebarProfile = profile => {
  ...
  if (!hasProfile) {
    if (neteaseSidebarName) {
      neteaseSidebarName.setAttribute('data-i18n', 'music.notSignedIn');
      neteaseSidebarName.textContent = tr('music.notSignedIn', {}, 'Not signed in');
    }
  ...
  if (neteaseSidebarName) {
    neteaseSidebarName.removeAttribute('data-i18n');
    neteaseSidebarName.textContent = profile.nickname || tr('music.neteaseCloud', {}, 'NetEase Cloud');
  }
...
const neteaseDownloadTaskFor = song => neteaseDownloadTasks.get(neteaseNumericId(song));
const neteaseDownloadStageText = stage => {
  if (stage === 'resolving') return tr('music.downloadPreparing', {}, 'Preparing download');
  if (stage === 'downloading') return tr('music.downloadInProgress', {}, 'Downloading');
  if (stage === 'tagging') return tr('music.downloadTagging', {}, 'Writing song details');
  if (stage === 'importing') return tr('music.downloadImporting', {}, 'Adding to your library');
  return tr('music.downloadFailed', {}, 'Download failed');
};
const neteaseDownloadFailureText = task => {
  const reason = task?.message || tr('legacy.tryAgain', {}, 'Please try again.');
  const stage = neteaseDownloadStageText(task?.stage);
  return tr('music.downloadFailedAt', { stage, reason }, `Download failed while ${stage}: ${reason}`);
};
const neteaseDownloadTaskText = task => {
  if (!task) return '';
  if (task.status === 'queued' || task.status === 'resolving') return tr('music.downloadPreparing', {}, 'Preparing download');
  if (task.status === 'downloading') {
    if (task.totalBytes > 0) return tr('music.downloadProgress', { progress: `${Math.min(100, Math.round((task.receivedBytes || 0) / task.totalBytes * 100))}%` }, `${Math.min(100, Math.round((task.receivedBytes || 0) / task.totalBytes * 100))}% downloaded`);
    return tr('music.downloadInProgress', {}, 'Downloading');
  }
  if (task.status === 'tagging') return tr('music.downloadTagging', {}, 'Writing metadata');
  if (task.status === 'importing') return tr('music.downloadImporting', {}, 'Adding to library');
  if (task.status === 'completed') return task.warning
    ? tr('music.downloadCompletedWithWarnings', {}, 'Downloaded; some song details could not be written.')
    : task.downgraded
    ? tr('music.downloadQualityDowngraded', { quality: task.resolvedQuality || tr('music.downloadStandard', {}, 'Standard'), requested: task.requestedQuality || tr('music.downloadStandard', {}, 'Standard') }, 'Downloaded at the best available quality')
    : tr('music.downloadCompleted', {}, 'Downloaded');
  if (task.status === 'cancelled') return tr('music.downloadCancelled', {}, 'Download cancelled');
  return neteaseDownloadFailureText(task);
};
const updateNeteaseDownloadRows = () => {
  neteaseResults?.querySelectorAll('tbody tr[data-netease-id]').forEach(row => {
    const state = row.querySelector('.netease-download-state');
    if (!state) return;
    const task = neteaseDownloadTasks.get(String(row.dataset.neteaseId || ''));
    state.dataset.status = task?.status || '';
    state.textContent = neteaseDownloadTaskText(task);
    state.hidden = !task;
  });
};
...
const formatDownloadDate = value => {
  ...
  return Number.isNaN(date.getTime()) ? '' : new Intl.DateTimeFormat(document.documentElement.lang || undefined, { year: 'numeric', month: 'short', day: 'numeric' }).format(date);
};
const downloadTaskStatusText = task => {
  if (task.status === 'queued') return task.message === 'download_restored' ? tr('music.downloadRestored', {}, 'Restoring download…') : tr('music.downloadQueued', {}, 'Queued');
  if (task.status === 'failed') return tr('music.downloadFailedState', {}, 'Failed');
  return neteaseDownloadTaskText(task);
};
const renderDownloadCover = (node, coverUrl) => {
...
const createDownloadMoreAction = ({ label, title, onSelect }) => {
  const cell = document.createElement('td');
  cell.className = 'track-actions netease-download-action';
  const wrap = document.createElement('span');
  wrap.className = 'track-row-menu-wrap';
  const trigger = document.createElement('button');
  trigger.className = 'track-row-menu-trigger';
  trigger.type = 'button';
  trigger.setAttribute('aria-label', tr('music.moreActions', {}, 'More actions'));
  trigger.setAttribute('aria-expanded', 'false');
  trigger.innerHTML = '<span class="material-symbols-outlined">more_horiz</span>';
  const menu = document.createElement('span');
  menu.className = 'track-row-menu';
  menu.hidden = true;
  const item = document.createElement('button');
  item.type = 'button';
  item.className = 'danger';
  item.innerHTML = '<span class="material-symbols-outlined">delete</span><span></span>';
  item.querySelector('span:last-child').textContent = label;
  trigger.addEventListener('click', event => { event.stopPropagation(); document.querySelectorAll('.track-row-menu').forEach(other => { if (other !== menu) other.hidden = true; }); menu.hidden = !menu.hidden; trigger.setAttribute('aria-expanded', String(!menu.hidden)); });
  item.addEventListener('click', async event => { event.stopPropagation(); menu.hidden = true; trigger.setAttribute('aria-expanded', 'false'); await onSelect(title); });
  menu.append(item); wrap.append(trigger, menu); cell.append(wrap);
  return cell;
};
const downloadedNeteaseTracks = state => (Array.isArray(state?.tracks) ? state.tracks : [])
  .filter(track => track.source === 'netease-download')
  .sort((left, right) => String(right.downloadedAt || right.addedAt || '').localeCompare(String(left.downloadedAt || left.addedAt || '')));
const playDownloadedTrack = async trackId => {
  const tracks = downloadedNeteaseTracks(musicState).filter(track => track.available !== false);
  ...
  if (playbackState.currentTrackId === target.id && playbackState.playing) {
    ...
    if (desktopMusic?.updatePlayback) musicState = await desktopMusic.updatePlayback({ currentTrackId: target.id, playing: false });
    notifyMusicPlayer({ currentTrackId: target.id, playing: false });
    return;
  }
  const queueTrackIds = tracks.map(track => track.id);
  if (desktopMusic?.updatePlayback) musicState = await desktopMusic.updatePlayback({ queueTrackIds, currentTrackId: target.id, playing: true, mode: 'sequence' });
...
const renderNeteaseDownloadPage = async () => {
  const api = getDesktopNetease();
  const desktopMusic = getDesktopMusic();
  if (!api?.getDownloadState || !neteaseAccountPanel || !neteaseResults) return;
  const requestId = ++neteaseAccountRequestId;
  setNeteaseSearchVisible(false);
  neteaseAccountPanel.innerHTML = '<div class="netease-download-head"><h1 class="playlist-detail-title"></h1><div class="netease-download-tabs" role="tablist"></div></div>';
  neteaseAccountPanel.querySelector('.playlist-detail-title').textContent = tr('music.download', {}, 'Download');
  neteaseResults.innerHTML = renderSongRowsLoading(tr('music.downloadInProgress', {}, 'Downloading…'));
  try {
    const [downloadState, localState] = await Promise.all([api.getDownloadState(), desktopMusic?.getState?.()]);
    if (neteaseActiveView !== 'download' || requestId !== neteaseAccountRequestId) return;
    if (localState) musicState = localState;
    neteaseDownloadTaskList = Array.isArray(downloadState?.tasks) ? downloadState.tasks : [];
    neteaseDownloadTasks.clear();
    neteaseDownloadTaskList.forEach(task => neteaseDownloadTasks.set(String(task.songId), task));
    const tabs = neteaseAccountPanel.querySelector('.netease-download-tabs');
    const panelId = 'neteaseDownloadResults';
    const tabDefs = [['downloaded', tr('music.downloaded', {}, 'Downloaded')], ['downloading', tr('music.downloading', {}, 'Downloading')]];
    tabs.replaceChildren(...tabDefs.map(([value, label]) => {
      const button = document.createElement('button');
      button.className = 'netease-download-tab';
      button.type = 'button';
      button.id = `netease-download-tab-${value}`;
      button.setAttribute('role', 'tab');
      button.setAttribute('aria-controls', panelId);
      button.setAttribute('aria-selected', String(neteaseDownloadTab === value));
      button.tabIndex = neteaseDownloadTab === value ? 0 : -1;
      button.textContent = label;
      button.addEventListener('click', () => { neteaseDownloadTab = value; renderNeteaseDownloadPage(); });
      ...
      return button;
    }), (() => { const indicator = document.createElement('span'); indicator.className = 'netease-download-indicator'; indicator.setAttribute('aria-hidden', 'true'); return indicator; })());
    requestAnimationFrame(() => { const active = tabs.querySelector('[aria-selected="true"]'); const indicator = tabs.querySelector('.netease-download-indicator'); if (active && indicator) { indicator.style.width = `${active.offsetWidth}px`; indicator.style.transform = `translateX(${active.offsetLeft}px)`; } });
    const panel = document.createElement('div');
    panel.id = panelId;
    panel.className = 'netease-download-panel';
    panel.setAttribute('role', 'tabpanel');
    panel.setAttribute('aria-labelledby', `netease-download-tab-${neteaseDownloadTab}`);
    if (neteaseDownloadTab === 'downloading') {
      const tasks = neteaseDownloadTaskList.filter(task => task.status !== 'completed');
      if (!tasks.length) { panel.classList.add('is-empty'); panel.innerHTML = `<p class="netease-empty">${tr('music.downloadQueueEmpty', {}, 'No downloads in progress.')}</p>`; }
      else {
        const table = document.createElement('table'); table.className = 'playlist-detail-table netease-download-table is-downloading';
        table.innerHTML = '<thead><tr><th class="track-index">#</th><th>Title</th><th>Quality</th><th>Status</th><th>Progress</th><th class="track-actions"></th></tr></thead><tbody></tbody>';
        const headers = table.querySelectorAll('th'); ['#', tr('music.title', {}, 'Title'), tr('music.downloadQualityLabel', {}, 'Quality'), tr('music.downloadStatus', {}, 'Status'), tr('music.downloadProgressLabel', {}, 'Progress'), ''].forEach((label, index) => { headers[index].textContent = label; });
        const body = table.querySelector('tbody');
        tasks.forEach((task, index) => {
          const row = document.createElement('tr');
          const indexCell = document.createElement('td'); indexCell.className = 'track-play-cell'; indexCell.innerHTML = '<span class="track-number"></span>'; indexCell.firstChild.textContent = String(index + 1).padStart(2, '0');
          const title = document.createElement('td'); title.innerHTML = '<div class="track-title-cell"><span class="track-cover"></span><span class="track-main"><span class="track-name"></span><span class="track-artist"></span></span></div>';
          renderDownloadCover(title.querySelector('.track-cover'), task.coverUrl); title.querySelector('.track-name').textContent = cleanMusicText(task.title) || tr('music.neteaseSong', {}, 'NetEase song'); title.querySelector('.track-artist').textContent = cleanMusicText(task.artist) || task.songId;
          const quality = document.createElement('td'); quality.textContent = task.downgraded && task.resolvedQuality ? `${task.requestedQuality} → ${task.resolvedQuality}` : task.resolvedQuality || task.requestedQuality;
          const status = document.createElement('td'); status.innerHTML = '<span class="netease-download-status"></span>'; const statusNode = status.firstChild; statusNode.dataset.status = task.status; statusNode.textContent = downloadTaskStatusText(task); if (task.status === 'failed' && task.message) { const reason = document.createElement('small'); reason.textContent = task.message; statusNode.append(document.createElement('br'), reason); }
          const progress = document.createElement('td'); progress.innerHTML = '<span class="netease-download-progress"><span class="netease-download-progress-top"><span></span><span></span></span><span class="netease-download-progress-bar"><i></i></span></span>'; const progressRoot = progress.firstChild; const percent = task.totalBytes > 0 ? Math.min(100, Math.round((task.receivedBytes || 0) / task.totalBytes * 100)) : 0; progressRoot.querySelector('.netease-download-progress-top span:first-child').textContent = task.status === 'failed' ? tr('music.downloadFailedState', {}, 'Failed') : `${percent}%`; progressRoot.querySelector('.netease-download-progress-top span:last-child').textContent = task.totalBytes ? `${formatDownloadBytes(task.receivedBytes)} / ${formatDownloadBytes(task.totalBytes)}` : formatDownloadBytes(task.receivedBytes); progressRoot.querySelector('i').style.width = `${percent}%`; progressRoot.classList.toggle('is-indeterminate', !task.totalBytes && task.status !== 'failed');
          const action = document.createElement('td'); action.className = 'track-actions netease-download-action'; if (task.status === 'failed') { const retry = document.createElement('button'); retry.type = 'button'; retry.textContent = tr('music.downloadRetry', {}, 'Retry'); retry.addEventListener('click', async () => { const result = await api.retryDownload?.({ taskId: task.taskId }); if (!result?.ok) toast.error(tr('music.downloadRetryFailed', {}, 'Unable to retry this download.')); }); const remove = createDownloadMoreAction({ label: tr('music.downloadRemoveTask', {}, 'Remove record'), title: '', onSelect: async () => { if (!await kairosConfirmAlert({ title: tr('music.downloadRemoveTaskTitle', {}, 'Remove download record?'), description: tr('music.downloadRemoveTaskDescription', {}, 'This failed download and its temporary file will be removed.') })) return; const result = await api.removeDownloadTask?.({ taskId: task.taskId }); if (!result?.ok) toast.error(tr('music.removeFailed', {}, 'Remove failed')); else renderNeteaseDownloadPage(); } }); action.append(retry, remove.firstChild); }
          row.append(indexCell, title, quality, status, progress, action); body.append(row);
        });
        panel.append(table);
      }
    } else {
      const tracks = downloadedNeteaseTracks(musicState);
      if (!tracks.length) { panel.classList.add('is-empty'); panel.innerHTML = `<p class="netease-empty">${tr('music.downloadedEmpty', {}, 'No downloaded songs yet.')}</p>`; }
      else {
        const table = document.createElement('table'); table.className = 'playlist-detail-table netease-download-table'; table.innerHTML = '<thead><tr><th class="track-index">#</th><th>Title</th><th>Album</th><th>Quality</th><th>Size</th><th>Downloaded</th><th class="track-actions"></th></tr></thead><tbody></tbody>';
        const headers = table.querySelectorAll('th'); ['#', tr('music.title', {}, 'Title'), tr('music.album', {}, 'Album'), tr('music.downloadQualityLabel', {}, 'Quality'), tr('music.downloadSize', {}, 'Size'), tr('music.downloadedAt', {}, 'Downloaded'), ''].forEach((label, index) => { headers[index].textContent = label; });
        const body = table.querySelector('tbody');
        tracks.forEach((track, index) => {
          const row = document.createElement('tr');
          const indexCell = document.createElement('td'); indexCell.className = 'track-play-cell'; indexCell.innerHTML = '<span class="track-number"></span><button class="track-play-button send" type="button" data-state="play"><svg class="track-play-icon" aria-hidden="true" viewBox="0 0 24 24"><path d="M8.75 6.45c0-1.18 1.29-1.9 2.29-1.28l8.22 5.14c.94.59.94 1.96 0 2.55L11.04 18c-1 .62-2.29-.1-2.29-1.28V6.45Z" fill="currentColor"/></svg><svg class="track-pause-icon" aria-hidden="true" viewBox="0 0 24 24"><path d="M7.4 5.2h3.2c.66 0 1.2.54 1.2 1.2v11.2c0 .66-.54 1.2-1.2 1.2H7.4c-.66 0-1.2-.54-1.2-1.2V6.4c0-.66.54-1.2 1.2-1.2Zm6 0h3.2c.66 0 1.2.54 1.2 1.2v11.2c0 .66-.54 1.2-1.2 1.2h-3.2c-.66 0-1.2-.54-1.2-1.2V6.4c0-.66.54-1.2 1.2-1.2Z" fill="currentColor"/></svg></button>'; indexCell.querySelector('.track-number').textContent = String(index + 1).padStart(2, '0'); const play = indexCell.querySelector('.track-play-button'); play.setAttribute('aria-label', tr('player.playTrack', {}, 'Play track')); play.disabled = track.available === false; play.addEventListener('click', () => playDownloadedTrack(track.id).catch(error => toast.error(tr('music.playFailed', {}, 'Play failed'), error?.message || '')));
          const title = document.createElement('td'); title.innerHTML = '<div class="track-title-cell"><span class="track-cover"></span><span class="track-main"><span class="track-name"></span><span class="track-artist"></span></span></div>'; renderDownloadCover(title.querySelector('.track-cover'), track.coverUrl); title.querySelector('.track-name').textContent = cleanMusicText(track.title) || cleanMusicText(track.fileName); title.querySelector('.track-artist').textContent = track.available === false ? tr('music.downloadMissing', {}, 'Missing') : cleanMusicText(track.artist); if (track.available === false) title.querySelector('.track-artist').classList.add('netease-download-missing');
          const album = document.createElement('td'); album.className = 'track-album'; album.textContent = cleanMusicText(track.album);
          const quality = document.createElement('td'); quality.textContent = [track.resolvedQuality || track.requestedQuality, track.format].filter(Boolean).join(' · ');
          const size = document.createElement('td'); size.textContent = formatDownloadBytes(track.fileSize);
          const date = document.createElement('td'); date.textContent = formatDownloadDate(track.downloadedAt || track.addedAt);
          const action = createDownloadMoreAction({ label: tr('music.removeDownloaded', {}, 'Remove'), title: track.title || track.fileName, onSelect: async () => { if (!await kairosConfirmAlert({ title: tr('music.removeDownloadedTitle', {}, 'Remove downloaded song?'), description: tr('music.removeDownloadedDescription', { title: track.title || track.fileName }, `${track.title || track.fileName} and its audio file will be removed from Kairos Downloads.`) })) return; try { const wasPlaying = playbackState.currentTrackId === track.id; if (wasPlaying) { const queueTrackIds = (musicState.queueTrackIds || []).filter(id => id !== track.id); if (desktopMusic?.updatePlayback) musicState = await desktopMusic.updatePlayback({ queueTrackIds, currentTrackId: null, playing: false }); playbackState = { currentTrackId: null, playing: false }; notifyMusicPlayer(localPlaybackDetail({ queueTrackIds, currentTrackId: null, playing: false })); } const result = await api.removeCompletedDownload?.({ trackId: track.id }); if (!result?.ok) throw new Error(result?.reason || 'remove_failed'); musicState = result.state || musicState; notifyMusicPlayer(localPlaybackDetail({ queueTrackIds: musicState.queueTrackIds || [], currentTrackId: musicState.currentTrackId || null, playing: false })); renderNeteaseDownloadPage(); } catch (error) { toast.error(tr('music.removeFailed', {}, 'Remove failed'), error?.message || ''); } } });
          row.append(indexCell, title, album, quality, size, date, action); body.append(row);
        });
        panel.append(table);
      }
    }
    neteaseResults.replaceChildren(panel);
  } catch (error) {
    if (neteaseActiveView === 'download' && requestId === neteaseAccountRequestId) neteaseResults.innerHTML = `<p class="netease-empty">${error?.message || tr('music.downloadFailed', {}, 'Download failed')}</p>`;
  }
};
const handleNeteaseDownloadProgress = (task, { notify = true } = {}) => {
  if (!task?.songId) return;
  const previousStatus = neteaseDownloadTasks.get(String(task.songId))?.status || '';
  neteaseDownloadTasks.set(String(task.songId), task);
  const index = neteaseDownloadTaskList.findIndex(item => item.taskId === task.taskId);
...
const subscribeNeteaseDownloadEvents = () => {
  if (neteaseDownloadUnsubscribe) return;
  const api = getDesktopNetease();
  if (!api?.onDownloadProgress) return;
  neteaseDownloadUnsubscribe = api.onDownloadProgress(handleNeteaseDownloadProgress);
  api.getDownloadState?.().then(state => {
    neteaseDownloadTaskList = Array.isArray(state?.tasks) ? state.tasks : state?.batches?.flatMap(batch => batch.tasks || []) || [];
    neteaseDownloadTaskList.forEach(task => handleNeteaseDownloadProgress(task, { notify: false }));
  }).catch(() => {});
};
const openNeteaseDownloadDialog = async songs => {
  const api = getDesktopNetease();
  const songIds = [...new Set((songs || []).map(neteaseNumericId).filter(id => /^\d{1,20}$/.test(id)))];
  if (!songIds.length || !api?.startDownload) return;
  try {
    const status = await api.getStatus?.();
    if (!status?.loggedIn) {
      toast.message(tr('music.downloadLoginRequired', {}, 'Log in to download songs.'), tr('music.loginAndTryAgain', {}, 'Please log in and try again.'));
      return;
    }
  } catch {
    toast.error(tr('music.downloadFailed', {}, 'Download failed'), tr('music.unableToCheckNeteaseStatus', {}, 'Unable to check NetEase status.'));
    return;
  }
  let dialog = document.getElementById('neteaseDownloadDialog');
  if (!dialog) {
    dialog = document.createElement('dialog');
    dialog.id = 'neteaseDownloadDialog';
    dialog.className = 'netease-download-dialog';
    dialog.innerHTML = `<form method="dialog"><h2></h2><p class="netease-download-description"></p><div class="netease-download-options"></div><footer class="netease-download-footer"><button type="submit" value="cancel"></button><button class="netease-download-submit" type="submit" value="confirm"></button></footer></form>`;
    document.body.append(dialog);
  }
  const form = dialog.querySelector('form');
  const qualities = [
    ['standard', tr('music.downloadStandard', {}, 'Standard'), tr('music.downloadStandardDescription', {}, 'Reliable quality and smaller files')],
    ['lossless', tr('music.downloadLossless', {}, 'Lossless'), tr('music.downloadLosslessDescription', {}, 'Higher fidelity when available')],
    ['hires', tr('music.downloadHires', {}, 'Hi-Res'), tr('music.downloadHiresDescription', {}, 'Best available quality; falls back automatically')]
  ];
  let quality = 'standard';
  dialog.querySelector('h2').textContent = tr('music.downloadSongsTitle', {}, 'Download songs');
  dialog.querySelector('.netease-download-description').textContent = tr('music.downloadQualityDescription', { count: songIds.length }, `Choose quality for ${songIds.length} songs. Higher tiers fall back automatically when unavailable.`);
  dialog.querySelector('.netease-download-footer button[value="cancel"]').textContent = tr('common.cancel', {}, 'Cancel');
  dialog.querySelector('.netease-download-submit').textContent = tr('music.startDownload', {}, 'Start download');
  const options = dialog.querySelector('.netease-download-options');
  options.replaceChildren(...qualities.map(([value, label, description]) => {
    const button = document.createElement('button');
    button.className = 'netease-download-quality';
    button.type = 'button';
    button.dataset.quality = value;
    button.setAttribute('aria-pressed', String(value === quality));
    button.innerHTML = `<span class="material-symbols-outlined">${value === 'hires' ? 'graphic_eq' : 'download'}</span><span class="netease-download-quality-copy"><strong></strong><span></span></span>`;
    button.querySelector('strong').textContent = label;
    button.querySelector('.netease-download-quality-copy span').textContent = description;
  ...
  form.onsubmit = async event => {
    ...
    event.preventDefault();
    const submit = dialog.querySelector('.netease-download-submit');
    submit.disabled = true;
    try {
      const result = await api.startDownload({ songIds, quality, songs: songs.map(song => ({ neteaseId: neteaseNumericId(song), title: song.title, artist: song.artist, album: song.album, coverUrl: song.coverUrl })) });
      if (!result?.ok) throw new Error(result?.message || tr('music.downloadFailed', {}, 'Download failed'));
      dialog.close('confirm');
      toast.success(tr('music.downloadInProgress', {}, 'Downloading'), musicCount('selectedCount', songIds.length, `${songIds.length} selected`));
    } catch (error) {
      toast.error(tr('music.downloadFailed', {}, 'Download failed'), error?.message || tr('legacy.tryAgain', {}, 'Please try again.'));
    } finally {
...
const updateNeteaseLikeRows = () => {
  neteaseResults?.querySelectorAll('tbody tr[data-id]').forEach(row => {
    ...
    if (menuLike) menuLike.textContent = likedMenuLabel(liked);
  });
};
...
const setNeteaseSongsLiked = async (songs, liked) => {
  const api = getDesktopNetease();
  const uniqueSongs = [...new Map((songs || []).map(song => [neteaseNumericId(song), song]).filter(([id]) => Boolean(id))).values()];
  if (!uniqueSongs.length || !api?.setSongLiked) return { succeededIds: new Set(), failed: 0 };
  if (!neteaseLikedIdsLoaded) await ensureNeteaseLikedIds().catch(() => {});
  const previousLiked = new Map(uniqueSongs.map(song => {
    const id = neteaseNumericId(song);
    return [id, neteaseLikedIdsLoaded ? neteaseLikedIds.has(id) : isNeteaseSongLiked(song)];
  }));
  uniqueSongs.forEach(song => {
    const id = neteaseNumericId(song);
    liked ? neteaseLikedIds.add(id) : neteaseLikedIds.delete(id);
    syncNeteaseCachedLike(id, liked);
    song.liked = liked;
  });
  updateNeteaseLikeRows();
  const results = await Promise.allSettled(uniqueSongs.map(song => api.setSongLiked({ id: neteaseNumericId(song), liked })));
  ...
  results.forEach((result, index) => {
    ...
    const previous = previousLiked.get(id) === true;
    previous ? neteaseLikedIds.add(id) : neteaseLikedIds.delete(id);
    syncNeteaseCachedLike(id, previous);
    song.liked = previous;
  });
  updateNeteaseLikeRows();
  const failed = uniqueSongs.length - succeededIds.size;
  if (failed) toast.error(tr('music.likeFailed', {}, 'Like failed'), tr('music.bulkActionPartialFailure', { count: failed }, `${failed} songs could not be updated.`));
  else toast[liked ? 'success' : 'message'](liked ? tr('music.neteaseLikedAdded', {}, 'Added to NetEase liked songs') : tr('music.neteaseLikedRemoved', {}, 'Removed from NetEase liked songs'));
...
const renderNeteaseResults = (songs, options = {}) => {
  ...
  const downloadSelection = options.downloadSelection || null;
  const table = document.createElement('table');
  table.className = 'playlist-detail-table netease-results';
  table.classList.toggle('is-selecting', Boolean(downloadSelection));
  ...
  songs.forEach((song, index) => {
    ...
    selectButton.setAttribute('aria-label', tr('music.selectSongs', {}, 'Select songs'));
    selectButton.setAttribute('aria-pressed', String(downloadSelection?.selected?.has(neteaseNumericId(song))));
    selectButton.innerHTML = '<span class="checkmark">check</span>';
    selectButton.addEventListener('click', event => {
      event.preventDefault();
      event.stopPropagation();
      downloadSelection?.onToggle?.(song);
    });
    playCell.append(number, playButton, selectButton);

    const titleCell = document.createElement('td');
    titleCell.innerHTML = '<div class="track-title-cell"><span class="track-cover"></span><span class="track-main"><span class="track-name"></span><span class="track-artist"></span><span class="netease-download-state" hidden></span></span></div>';
    ...
    const downloadState = titleCell.querySelector('.netease-download-state');
    const downloadTask = neteaseDownloadTaskFor(song);
    downloadState.dataset.status = downloadTask?.status || '';
    downloadState.textContent = neteaseDownloadTaskText(downloadTask);
    downloadState.hidden = !downloadTask;
    ...
    menu.querySelectorAll('button[data-action]').forEach(button => {
      button.addEventListener('click', async event => {
        ...
        trigger.setAttribute('aria-expanded', 'false');
        try {
          if (button.dataset.action === 'download') await openNeteaseDownloadDialog([song]);
          else if (button.dataset.action === 'like') await toggleNeteaseLike(song, likeButton);
  ...
  const selectionBar = downloadSelection ? document.createElement('div') : null;
  if (selectionBar) {
    selectionBar.className = 'playlist-selection-bar';
    const selectedCount = downloadSelection.selected?.size || 0;
    const selectedSongs = songs.filter(song => downloadSelection.selected?.has(neteaseNumericId(song)));
    const nextLiked = !selectedSongs.length || !selectedSongs.every(isNeteaseSongLiked);
    selectionBar.innerHTML = `<span class="playlist-selection-text">${musicCount('selectedCount', selectedCount, `${selectedCount} selected`)}</span><span class="playlist-selection-actions"><button type="button" data-action="cancel">${tr('common.cancel', {}, 'Cancel')}</button><button type="button" data-action="queue" ${selectedCount ? '' : 'disabled'}>${tr('music.addToQueue', {}, 'Add to Queue')}</button><button type="button" data-action="like" ${selectedCount ? '' : 'disabled'}>${likedMenuLabel(!nextLiked)}</button><button type="button" data-action="download" ${selectedCount ? '' : 'disabled'}>${tr('music.download', {}, 'Download')}</button></span>`;
    selectionBar.querySelector('[data-action="cancel"]').addEventListener('click', () => downloadSelection.onCancel?.());
    selectionBar.querySelector('[data-action="queue"]').addEventListener('click', () => downloadSelection.onQueue?.(selectedSongs));
    selectionBar.querySelector('[data-action="like"]').addEventListener('click', () => downloadSelection.onLike?.(selectedSongs, nextLiked));
    selectionBar.querySelector('[data-action="download"]').addEventListener('click', () => downloadSelection.onDownload?.());
  }
  if (options.withFilter) {
    ...
    const historyToggle = options.historyType === 0 || options.historyType === 1
      ? `<span class="netease-history-tabs" role="tablist" aria-label="${tr('music.historyRange', {}, 'History range')}">
          <button class="netease-history-tab" type="button" role="tab" data-history-type="1" aria-selected="${options.historyType === 1}" tabindex="${options.historyType === 1 ? '0' : '-1'}">${tr('music.historyRecent', {}, 'Recent week')}</button>
          <button class="netease-history-tab" type="button" role="tab" data-history-type="0" aria-selected="${options.historyType === 0}" tabindex="${options.historyType === 0 ? '0' : '-1'}">${tr('music.historyAllTime', {}, 'All time')}</button>
    ...
    const positionHistoryIndicator = () => {
      ...
      const active = tabs.querySelector('.netease-history-tab[aria-selected="true"]');
    ...
    tabs.querySelectorAll('[data-history-type]').forEach(button => {
      button.addEventListener('click', () => {
        ...
        tabs.querySelectorAll('[data-history-type]').forEach(item => { item.setAttribute('aria-selected', String(item === button)); item.tabIndex = item === button ? 0 : -1; });
        positionHistoryIndicator();
        neteaseHistoryType = nextType;
        loadNeteaseHistory(nextType);
      });
      tabs.querySelectorAll('[data-history-type]').forEach(button => button.addEventListener('keydown', event => {
        if (!['ArrowLeft', 'ArrowRight', 'Home', 'End'].includes(event.key)) return;
        event.preventDefault();
        const buttons = [...tabs.querySelectorAll('[data-history-type]')];
        const index = buttons.indexOf(button);
        const next = event.key === 'Home' ? 0 : event.key === 'End' ? buttons.length - 1 : (index + (event.key === 'ArrowRight' ? 1 : -1) + buttons.length) % buttons.length;
        buttons[next]?.focus();
        buttons[next]?.click();
    ...
    neteaseResults.replaceChildren(...(selectionBar ? [selectionBar, tabs, table] : [tabs, table]));
  } else {
    neteaseResults.replaceChildren(...(selectionBar ? [selectionBar, table] : [table]));
  }
...
const createNeteaseHomeSongTable = songs => {
  ...
  songs.forEach((song, index) => {
    ...
    row.innerHTML = `
      ...
      <td><div class="track-title-cell"><span class="track-cover"></span><span class="track-main"><span class="track-name"></span><span class="track-artist"></span><span class="netease-download-state" hidden></span></span></div></td>
    ...
    const downloadState = row.querySelector('.netease-download-state');
    const downloadTask = neteaseDownloadTaskFor(song);
    downloadState.dataset.status = downloadTask?.status || '';
    downloadState.textContent = neteaseDownloadTaskText(downloadTask);
    downloadState.hidden = !downloadTask;
...
const renderNeteaseSongCollection = (title, songs = [], subtitle = '', options = {}) => {
  ...
  setNeteaseSearchVisible(false);
  let downloadSelectMode = false;
  const selectedDownloadSongIds = new Set();
  const renderCollectionSongs = () => {
    ...
    renderNeteaseResults(songs, {
      remember: false,
      queueSongs: songs,
      withFilter: true,
      showPlayCount: options.showPlayCount === true,
      historyType: options.historyType,
      emptyMessage: options.emptyMessage,
      downloadSelection: {
      selected: selectedDownloadSongIds,
      onToggle: song => {
        const id = neteaseNumericId(song);
        if (selectedDownloadSongIds.has(id)) selectedDownloadSongIds.delete(id);
        else selectedDownloadSongIds.add(id);
        renderCollectionSongs();
      },
      onCancel: () => {
      ...
      onQueue: async selectedSongs => {
        try {
          for (const song of selectedSongs) await enqueueNeteaseTrack(song, 'queue');
          toast.success(tr('music.addToQueue', {}, 'Add to Queue'), musicCount('selectedCount', selectedSongs.length, `${selectedSongs.length} selected`));
        } catch (error) {
          console.error('netease bulk queue failed', error);
          toast.error(tr('music.addFailed', {}, 'Add failed'), error?.message || tr('music.songUnavailable', {}, 'This song is unavailable.'));
        }
      },
      onLike: async (selectedSongs, liked) => {
        const { succeededIds } = await setNeteaseSongsLiked(selectedSongs, liked);
        if (!liked && neteaseActiveView === 'liked' && succeededIds.size) {
          for (let index = songs.length - 1; index >= 0; index -= 1) {
            if (succeededIds.has(neteaseNumericId(songs[index]))) songs.splice(index, 1);
          }
          neteaseLastSongs = neteaseLastSongs.filter(song => !succeededIds.has(neteaseNumericId(song)));
          neteaseVisibleSongs = neteaseVisibleSongs.filter(song => !succeededIds.has(neteaseNumericId(song)));
        }
        selectedDownloadSongIds.clear();
        downloadSelectMode = false;
        renderCollectionSongs();
      },
      onDownload: () => openNeteaseDownloadDialog(songs.filter(song => selectedDownloadSongIds.has(neteaseNumericId(song))))
      }
    });
  };
  if (neteaseAccountPanel) {
    ...
    hero.innerHTML = `
      ...
      <div>
        ...
        <div class="playlist-detail-actions">
          ...
          ${options.playlist ? `<button class="playlist-subscribe-button playlist-download-select" type="button"><span class="material-symbols-outlined">checklist</span><span>${tr('music.selected', {}, 'Selected')}</span></button>` : ''}
    ...
    const subscribeButton = hero.querySelector('.playlist-subscribe-button[data-subscribed]');
    ...
    hero.querySelector('.playlist-download-select')?.addEventListener('click', () => {
...
const showNeteaseSection = (view = 'search') => {
  ...
  if (view === 'download') renderNeteaseDownloadPage();
};
...
const renderPlaylistDetail = playlistId => {
  ...
  table.innerHTML = `
    <thead><tr><th class="track-index">#</th><th>${tr('music.title', {}, 'Title')}</th><th>${tr('music.album', {}, 'Album')}</th><th class="track-like">${tr('music.like', {}, 'Like')}</th><th class="track-duration">${tr('music.time', {}, 'Time')}</th><th class="track-actions"></th></tr></thead>
    <tbody></tbody>
  `;
  const tbody = table.querySelector('tbody');
  ...
  const updateSelectionBar = () => {
    if (detailMode === 'select') {
      ...
      selectionBar.querySelector('[data-action="apply"]').textContent = isLikedPlaylist ? tr('music.unliked', {}, 'Unliked') : tr('music.removeSelected', {}, 'Remove Selected');
  ...
  const renderRows = query => {
    ...
    filtered.forEach((track, index) => {
      ...
      <td class="track-actions"><span class="track-row-menu-wrap"><button class="track-row-menu-trigger" type="button" aria-label="${tr('music.moreActions', {}, 'More actions')}" aria-expanded="false"><span class="material-symbols-outlined">more_horiz</span></button><span class="track-row-menu" hidden>${unavailable ? '' : `<button type="button" data-action="play-next"><span class="material-symbols-outlined">queue_play_next</span><span>${tr('music.addToPlayNext', {}, 'Play Next')}</span></button>`}${isLikedPlaylist ? '' : `<button type="button" data-action="like"><span class="material-symbols-outlined">favorite</span><span>${likedMenuLabel(track.liked)}</span></button>`}<button class="danger" type="button" data-action="remove"><span class="material-symbols-outlined">${isLikedPlaylist ? 'heart_minus' : 'visibility_off'}</span><span>${isLikedPlaylist ? tr('music.unliked', {}, 'Unliked') : tr('music.removeFromPlaylist', {}, 'Remove from Playlist')}</span></button></span></span></td>
        `;
        const selectButton = row.querySelector('.track-select-button');
        if (selectButton) {
          selectButton.addEventListener('click', () => {
...
subscribeNeteaseDownloadEvents();
showInitialMusicSource();
