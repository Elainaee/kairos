(() => {
  ...
  const calendarBackgroundMarkup = () => {
    ...
    const cards = [...calendarBackgrounds];
    if (!cards.some(item => item.id === background.id)) cards.unshift({ source: 'custom', id: background.id, label: 'Default' });
  ...
  const remindersMarkup = () => sectionView('reminders', 'Reminders', `
    ${card('Default Rules', `
      <div class="kairos-settings-group">
        ...
        ${toggle('kairosDesktopNotifications', 'Windows notifications', state.reminders.desktopNotifications, 'reminders.desktopNotifications', 'When off, in-app reminders and the desktop pet stay available.')}
      </div>`)}
  `);
  ...
  const musicMarkup = () => {
    ...
    const brand = '<span class="kairos-netease-brand"><img src="../../assets/icons/netease-format.ico" alt="" aria-hidden="true">Netease Music</span>';
    const controls = `
    ...
    dialog.innerHTML = `
      <form method="dialog" class="kairos-settings-panel">
        ...
        <div class="kairos-settings-content">
          ...
          ${sectionView('appearance', 'Appearance', `<div class="kairos-settings-group">${select('kairosTheme', 'Theme mode', state.appearance.theme, choices.theme, 'appearance.theme')}${toggle('kairosReduceMotion', 'Reduce motion', state.accessibility.reduceMotion, 'accessibility.reduceMotion')}</div>${calendarBackgroundMarkup()}`)}
          ${remindersMarkup()}
          ${habitsMarkup()}
          ${agentMarkup()}
          ${musicMarkup()}
  ...
  const refreshProviderData = async () => {
    ...
    [providerCatalog, providerSettings, neteaseStatus, appBackups, appAudit, calendarBackgrounds] = await Promise.all([
