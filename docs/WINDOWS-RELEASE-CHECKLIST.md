...
- [ ] 网易云入口可打开，未登录状态有清晰提示。
- [ ] 桌宠 / AI 入口可打开 AI 窗口。
- [ ] 在 Settings > Reminders 确认 Windows notifications 默认开启；创建一条几分钟后到期的测试日程，确认同时出现应用内提醒、桌宠 提示和 Windows 通知中心提醒；点击系统通知后确认 Kairos 聚焦并打开对应日程编辑窗口。关闭该选项后再次触发，确认仅保留应用内提醒和 桌宠 提示。
...
5. 在原生菜单中显示/隐藏 桌宠，确认角色窗口与主视图状态一致。
