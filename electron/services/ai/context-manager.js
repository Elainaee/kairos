...
export class ContextManager{
  ...
  buildSummaryPrompt(messages,{locale="en"}={}){const instruction=locale==="zh-CN"?"请将以下对话压缩成可供后续模型继续交流的事实摘要。保留用户偏好、未完成事项、明确日期、关键结论和待确认操作；不要补充原文没有的信息。":"Summarize the following conversation into factual context for a later model. Preserve user preferences, unfinished work, explicit dates, key conclusions, and pending actions; do not add information that is not present.";return[{role:"user",content:`${instruction}\n\n${messages.map(x=>`${x.role}: ${x.content}`).join("\n")}`}];}
  ...
  async createContinuation(conversationId,{carrySummary=false,titleSuffix=" · Continue"}={}){const source=await this.store.getConversation(conversationId);if(!source)throw new Error("conversation_not_found");return this.store.createConversation({title:`${source.title}${titleSuffix}`,provider:source.provider,model:source.model,summary:carrySummary?source.summary:""});}
}
