...
test("Windows app.asar contains runtime files without local secrets or source docs", async () => {
  ...
  assert.ok(entries.includes("app/index.html"), "main renderer should be bundled");
  assert.ok(entries.includes("electron/main.js"), "main process should be bundled");
...
async function verifyExecutableStartup(executablePath) {
  ...
  const result = await runSmokeTest(executablePath, {
  ...
  assert.match(result.stdout, /Kairos smoke test loaded main window\./);
  ...
  const second = await runSmokeTest(executablePath, {
  ...
  assert.match(second.stdout, /Kairos smoke test loaded main window\./);
  ...
  const recovered = await runSmokeTest(executablePath, {
    env: { KAIROS_USER_DATA_DIR: userDataDir, KAIROS_SMOKE_RESULT_PATH: recoveredResultPath, KAIROS_SMOKE_STATE_MARKER: marker, KAIROS_SMOKE_EXPECT_STATE_MARKER: "1", KAIROS_SMOKE_MUSIC_FILE: musicFile }
  });
  assert.match(recovered.stdout, /Kairos smoke test loaded main window\./);
  ...
  const recoveredMusic = await runSmokeTest(executablePath, {
    env: { KAIROS_USER_DATA_DIR: userDataDir, KAIROS_SMOKE_RESULT_PATH: recoveredMusicResultPath, KAIROS_SMOKE_STATE_MARKER: marker, KAIROS_SMOKE_EXPECT_STATE_MARKER: "1", KAIROS_SMOKE_MUSIC_FILE: musicFile }
  });
  assert.match(recoveredMusic.stdout, /Kairos smoke test loaded main window\./);
  ...
  const unavailableMusic = await runSmokeTest(executablePath, {
    env: { KAIROS_USER_DATA_DIR: userDataDir, KAIROS_SMOKE_RESULT_PATH: unavailableMusicResultPath, KAIROS_SMOKE_STATE_MARKER: marker, KAIROS_SMOKE_EXPECT_STATE_MARKER: "1", KAIROS_SMOKE_MUSIC_FILE: musicFile, KAIROS_SMOKE_EXPECT_MUSIC_MISSING: "1" }
  });
  assert.match(unavailableMusic.stdout, /Kairos smoke test loaded main window\./);
  ...
  const recoveredAi = await runSmokeTest(executablePath, {
    env: { KAIROS_USER_DATA_DIR: userDataDir, KAIROS_SMOKE_RESULT_PATH: recoveredAiResultPath, KAIROS_SMOKE_STATE_MARKER: marker, KAIROS_SMOKE_EXPECT_STATE_MARKER: "1", KAIROS_SMOKE_MUSIC_FILE: musicFile }
  });
  assert.match(recoveredAi.stdout, /Kairos smoke test loaded main window\./);
