...
const installerPath = path.join(releaseDir, `${packageJson.productName}-${packageJson.version}-win-x64-setup.exe`);
...
const installedExe = path.join(installDir, `${packageJson.productName}.exe`);
const uninstaller = path.join(installDir, `Uninstall ${packageJson.productName}.exe`);
let installed = false;
