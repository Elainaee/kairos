...
import { APP_STATE_SCHEMA_VERSION, AppStateRepository, AppStateStore, auditAppState, createAppAdapters, normalize } from "./app-state.js";
...
test("app state audit CLI reports healthy and invalid exports", async () => {
  ...
  const healthy = await runNode(["electron/app-state-audit.js", good]);
  const invalid = await runNode(["electron/app-state-audit.js", bad]);
...
test("desktop data audit summarizes required and optional userData files", async () => {
  ...
  const cli = await runNode(["electron/desktop-data-audit.js", dir]);
  await fs.rm(path.join(dir, "app-state.json"));
  const missing = await runNode(["electron/desktop-data-audit.js", dir]);
